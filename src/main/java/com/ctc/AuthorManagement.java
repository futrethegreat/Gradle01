...
public class AuthorManagement {
	/**
	 * Creates new Author in DB using object data. Checks if exists and if there are
	 * problems.
	 * 
	 * @author DavidSauce
	 * @version 11-03-2018
	...
	static void newAuthor() throws SQLException {
		...
		if (lAuthor.existsAuthorNameInDB(Main.c)) {
			Main.c.rollback();
			Utils.consoleMsg("WARNING: " + lAuthorName + " not created. Author already exists in DB.");
		} else {
			if (lAuthor.addAuthorToDB(Main.c) == 1) {
				Main.c.commit();
				System.out.println("INFO: " + lAuthorName + " created succesfully in DB.");
			} else {
				Main.c.rollback();
				System.out.println("ERROR: " + lAuthorName + " does not exist but NOT created in DB.");
			}
		}
	...
	/**
	 * Deletes Author in DB using object data. Checks if exists and if there are
	 * problems.
	 * 
	 * @author DavidSauce
	 * @version 11-03-2018
	...
	static void deleteAuthor() throws SQLException {
		...
		case 1:
			Main.c.commit();
			System.out.println("INFO: " + lAuthorID + " deleted SUCCESSFULLY.");
			break;
		case 0:
			Main.c.rollback();
			System.out.println("INFO: Author with ID:" + lAuthorID + " does not exist in DB. Nothing to delete.");
			break;
		case -1:
			Main.c.rollback();
			System.out.println("INFO: Author with ID:" + lAuthorID + " not deleted. Delete related books first.");
			break;
		default:
			Main.c.rollback();
			System.out.println("ERROR: Something happened deleting the Author using ID.");
			break;
		}
		Main.c.setAutoCommit(true);
	}
	...
	/**
	 * Modifies Author in DB using object data. Checks if exists and if there are
	 * problems.
	 * 
	 * @author DavidSauce
	 * @version 11-03-2018
	...
	static void modifyAuthor() throws SQLException {
		...
		if (lAuthor.getAuthorName() == null) {
			Main.c.rollback();
			System.out.println("INFO: Author with ID:" + lAuthorID + " does not exist in DB. Nothing to modify.");
		} else {
			...
			case 1:
				Main.c.commit();
				System.out.println("INFO: Author with ID: " + lAuthorID + " modified SUCCESSFULLY from: "
			...
			case -1:
				Main.c.rollback();
				System.out.println("INFO: Author with ID:" + lAuthorID + " not modified.");
				break;
			default:
				Main.c.rollback();
				System.out.println("ERROR: Something happened deleting the Author using ID.");
