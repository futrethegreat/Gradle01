...
public class Author {
  ...
  /**
   * Constructor: Creates an Author object using parameters pAuthorID and pAuthorName.
   * 
   * @author DavidSauce
   * @version 11-03-2018
   * @param pAuthorID
  ...
  /**
   * Constructor: Gets Author data from DB with pAuthorID and creates an Author object with DB data.
   * 
   * @author DavidSauce
   * @version 11-03-2018
   * @param pConnection
  ...
  /**
   * Constructor: Gets Author data from DB with pAuthorName and creates an Author object with DB
   * data.
   * 
   * @author DavidSauce
   * @version 11-03-2018
  ...
  public Author(Connection pConnection, String pAuthorName) {
    try {
      PreparedStatement s = pConnection
          .prepareStatement("SELECT * FROM authors WHERE AuthorName =  ?");
      ...
      System.out.println("ERROR: Constructor Author(Connection pConnection, String pAuthorName)");
  ...
  public int addAuthorToDB(Connection pConnection) throws SQLException {
    PreparedStatement s = pConnection
        .prepareStatement("INSERT INTO authors (AuthorName) VALUES ( ? )");
  ...
  public int modifyAuthorNameInDB(Connection pConnection) throws SQLException {
    try {
      PreparedStatement s = pConnection
          .prepareStatement("UPDATE authors SET AuthorName = ? WHERE AuthorID = ?");
      s.setString(1, this.getAuthorName());
